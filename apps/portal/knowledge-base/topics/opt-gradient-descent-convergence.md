<!-- GENERATED: knowledge-base -->
# 勾配降下法と収束

**Domain:** [最適化](../domains/optimization)
**Module:** Optimization Core
**Level:** introductory
**Status:** draft
**Tags:** optimization, optimization-core

## Prerequisites

- [line searchとstep size](/knowledge-base/topics/opt-line-search-step-size)

## 次に学ぶTopic

[momentumと加速勾配法](/knowledge-base/topics/opt-momentum-accelerated-gradient) / [Newton法と準Newton法](/knowledge-base/topics/opt-newton-quasi-newton) / [座標降下法と共役方向](/knowledge-base/topics/opt-coordinate-conjugate-directions) / [Mirror descentとBregman divergence](/knowledge-base/topics/opt-mirror-descent-bregman) / [確率的勾配法](/knowledge-base/topics/opt-stochastic-gradient) / [線形回帰](/knowledge-base/topics/ml-linear-regression) / [boostingとgradient boosting](/knowledge-base/topics/ml-boosting-gradient-boosting)

## Learning Path

[工学数学コア](../paths/engineering-mathematics-core) / [機械学習の数学](../paths/machine-learning-mathematics)

## Case Study

登録なし

## 教材

[テーマホーム](/courses/foundation/opt-gradient-descent-convergence) / [教科書](/textbook/opt-gradient-descent-convergence) / [演習](/exercises/opt-gradient-descent-convergence) / [スライド](/slides/opt-gradient-descent-convergence/)
