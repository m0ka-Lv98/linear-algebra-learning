...
export default defineConfig({
  ...
  themeConfig: {
    ...
    sidebar: {
      ...
      '/textbook/': [{ text: 'Course 00 教科書', items: items('textbook') }, { text: 'Course 01 教科書', items: [...calculusSingle, ...calculusMulti].map(([text, id]) => ({ text, link: `/textbook/${id}` })) }, { text: 'Course 02 教科書', items: linearAlgebra.map(([text, id]) => ({ text, link: `/textbook/${id}` })) }, { text: 'Course 03 教科書', items: probability.map(([text, id]) => ({ text, link: `/textbook/${id}` })) }],
      '/exercises/': [{ text: 'Course 00 演習', items: items('exercises') }, { text: 'Course 01 演習', items: [...calculusSingle, ...calculusMulti].map(([text, id]) => ({ text, link: `/exercises/${id}` })) }, { text: 'Course 02 演習', items: linearAlgebra.map(([text, id]) => ({ text, link: `/exercises/${id}` })) }, { text: 'Course 03 演習', items: probability.map(([text, id]) => ({ text, link: `/exercises/${id}` })) }]
    },
    socialLinks: []
  }
})
