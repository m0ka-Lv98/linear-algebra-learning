...
[教科書](/textbook/dl-rnn-sequence-models)と対応する10問。ヒント以降は折りたたんである。

### 09-907-01：概念

1. RNNを定義し、系列modelとの違いを説明せよ。
...
- 完全解答: RNNは対象と条件を定める概念であり、系列modelとは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: RNNと系列modelの定義と計算手順を同一視する。

</details>

### 09-907-02：概念

2. 「RNNと系列model」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\mathbf{h}_t=\phi(\mathbf{W}_h\mathbf{h}_{t-1}+\mathbf{W}_x\mathbf{x}_t)$$
...
6. 「RNNと系列modelの定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにRNNと系列modelの公式を適用した。どの項目を検査すべきか。
...
8. RNNと系列modelを実装した関数の入力・出力・検算項目を設計せよ。
...
9. RNNと系列modelが深層学習の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: RNNと系列modelは、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. RNNと系列modelの結果が不安定または不適切だった場合の診断手順を提案せよ。
