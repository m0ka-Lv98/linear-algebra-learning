...
[教科書](/textbook/dl-scaling-distributed-training)と対応する10問。ヒント以降は折りたたんである。

### 09-918-01：概念

1. scalingを定義し、分散学習との違いを説明せよ。
...
- 完全解答: scalingは対象と条件を定める概念であり、分散学習とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: scalingと分散学習の定義と計算手順を同一視する。

</details>

### 09-918-02：概念

2. 「scalingと分散学習」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\mathbf{g}=\frac{1}{P}\sum_{p=1}^{P}\mathbf{g}_p$$
...
6. 「scalingと分散学習の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにscalingと分散学習の公式を適用した。どの項目を検査すべきか。
...
8. scalingと分散学習を実装した関数の入力・出力・検算項目を設計せよ。
...
9. scalingと分散学習が深層学習の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: scalingと分散学習は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. scalingと分散学習の結果が不安定または不適切だった場合の診断手順を提案せよ。
