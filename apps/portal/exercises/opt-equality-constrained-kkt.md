...
[教科書](/textbook/opt-equality-constrained-kkt)と対応する10問。ヒント以降は折りたたんである。

### 06-611-01：概念

1. 等式制約を定義し、KKT条件との違いを説明せよ。
...
- 完全解答: 等式制約は対象と条件を定める概念であり、KKT条件とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 等式制約とKKT条件の定義と計算手順を同一視する。

</details>

### 06-611-02：概念

2. 「等式制約とKKT条件」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
6. 「等式制約とKKT条件の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに等式制約とKKT条件の公式を適用した。どの項目を検査すべきか。
...
8. 等式制約とKKT条件を実装した関数の入力・出力・検算項目を設計せよ。
...
9. 等式制約とKKT条件が最適化の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 等式制約とKKT条件は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 等式制約とKKT条件の結果が不安定または不適切だった場合の診断手順を提案せよ。
