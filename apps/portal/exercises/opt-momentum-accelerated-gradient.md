...
[教科書](/textbook/opt-momentum-accelerated-gradient)と対応する10問。ヒント以降は折りたたんである。

### 06-607-01：概念

1. momentumを定義し、加速勾配法との違いを説明せよ。
...
- 完全解答: momentumは対象と条件を定める概念であり、加速勾配法とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: momentumと加速勾配法の定義と計算手順を同一視する。

</details>

### 06-607-02：概念

2. 「momentumと加速勾配法」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\mathbf{v}_{k+1}=\beta\mathbf{v}_k+\nabla f(\mathbf{x}_k)$$
...
4. momentumと加速勾配法を最小の非自明な例で計算せよ。
...
5. momentumに関する性質を一つ選び、定義から導出せよ。
...
6. 「momentumと加速勾配法の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにmomentumと加速勾配法の公式を適用した。どの項目を検査すべきか。
...
8. momentumと加速勾配法を実装した関数の入力・出力・検算項目を設計せよ。
...
9. momentumと加速勾配法が最適化の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: momentumと加速勾配法は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. momentumと加速勾配法の結果が不安定または不適切だった場合の診断手順を提案せよ。
