...
[教科書](/textbook/num-ode-stability-stiffness)と対応する10問。ヒント以降は折りたたんである。

### 05-518-01：概念

1. ODEの安定性を定義し、硬さとの違いを説明せよ。
...
- 完全解答: ODEの安定性は対象と条件を定める概念であり、硬さとは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: ODEの安定性・硬さ・陰解法の定義と計算手順を同一視する。

</details>

### 05-518-02：概念

2. 「ODEの安定性・硬さ・陰解法」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$y_{k+1}=y_k+h\lambda y_{k+1}$$
...
4. ODEの安定性・硬さ・陰解法を最小の非自明な例で計算せよ。
...
6. 「ODEの安定性・硬さ・陰解法の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにODEの安定性・硬さ・陰解法の公式を適用した。どの項目を検査すべきか。
...
8. ODEの安定性・硬さ・陰解法を実装した関数の入力・出力・検算項目を設計せよ。
...
9. ODEの安定性・硬さ・陰解法が数値計算の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: ODEの安定性・硬さ・陰解法は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. ODEの安定性・硬さ・陰解法の結果が不安定または不適切だった場合の診断手順を提案せよ。
