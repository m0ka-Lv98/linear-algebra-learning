...
[教科書](/textbook/mat-convolution-linear-systems)と対応する10問。ヒント以降は折りたたんである。

### 07-712-01：概念

1. 畳み込みを定義し、線形時不変系との違いを説明せよ。
...
- 完全解答: 畳み込みは対象と条件を定める概念であり、線形時不変系とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 畳み込みと線形時不変系の定義と計算手順を同一視する。

</details>

### 07-712-02：概念

2. 「畳み込みと線形時不変系」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$(x*h)_n=\sum_k x_k h_{n-k}$$
...
6. 「畳み込みと線形時不変系の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに畳み込みと線形時不変系の公式を適用した。どの項目を検査すべきか。
...
8. 畳み込みと線形時不変系を実装した関数の入力・出力・検算項目を設計せよ。
...
9. 畳み込みと線形時不変系がデータ解析の行列手法の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 畳み込みと線形時不変系は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 畳み込みと線形時不変系の結果が不安定または不適切だった場合の診断手順を提案せよ。
