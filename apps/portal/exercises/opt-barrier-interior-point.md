...
[教科書](/textbook/opt-barrier-interior-point)と対応する10問。ヒント以降は折りたたんである。

### 06-614-01：概念

1. barrier法を定義し、内点法との違いを説明せよ。
...
- 完全解答: barrier法は対象と条件を定める概念であり、内点法とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: barrier法と内点法の定義と計算手順を同一視する。

</details>

### 06-614-02：概念

2. 「barrier法と内点法」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\phi(\mathbf{x})=-\sum_i\log(-g_i(\mathbf{x}))$$
...
5. barrier法に関する性質を一つ選び、定義から導出せよ。
...
6. 「barrier法と内点法の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにbarrier法と内点法の公式を適用した。どの項目を検査すべきか。
...
8. barrier法と内点法を実装した関数の入力・出力・検算項目を設計せよ。
...
9. barrier法と内点法が最適化の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: barrier法と内点法は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. barrier法と内点法の結果が不安定または不適切だった場合の診断手順を提案せよ。
