...
[教科書](/textbook/mat-tensors-multilinear-overview)と対応する10問。ヒント以降は折りたたんである。

### 07-720-01：概念

1. tensorを定義し、多重線形構造との違いを説明せよ。
...
- 完全解答: tensorは対象と条件を定める概念であり、多重線形構造とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: tensorと多重線形構造の定義と計算手順を同一視する。

</details>

### 07-720-02：概念

2. 「tensorと多重線形構造」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\mathcal{X}\approx\sum_{r=1}^{R}\mathbf{a}_r\circ\mathbf{b}_r\circ\mathbf{c}_r$$
...
6. 「tensorと多重線形構造の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにtensorと多重線形構造の公式を適用した。どの項目を検査すべきか。
...
8. tensorと多重線形構造を実装した関数の入力・出力・検算項目を設計せよ。
...
9. tensorと多重線形構造がデータ解析の行列手法の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: tensorと多重線形構造は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. tensorと多重線形構造の結果が不安定または不適切だった場合の診断手順を提案せよ。
