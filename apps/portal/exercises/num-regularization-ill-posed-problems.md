...
[教科書](/textbook/num-regularization-ill-posed-problems)と対応する10問。ヒント以降は折りたたんである。

### 05-515-01：概念

1. 正則化を定義し、悪条件との違いを説明せよ。
...
- 完全解答: 正則化は対象と条件を定める概念であり、悪条件とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 正則化と悪条件・不適切問題の定義と計算手順を同一視する。

</details>

### 05-515-02：概念

2. 「正則化と悪条件・不適切問題」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\min_{\mathbf{x}}\|\mathbf{A}\mathbf{x}-\mathbf{b}\|_2^2+\lambda\|\mathbf{x}\|_2^2$$
...
6. 「正則化と悪条件・不適切問題の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに正則化と悪条件・不適切問題の公式を適用した。どの項目を検査すべきか。
...
8. 正則化と悪条件・不適切問題を実装した関数の入力・出力・検算項目を設計せよ。
...
9. 正則化と悪条件・不適切問題が数値計算の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 正則化と悪条件・不適切問題は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 正則化と悪条件・不適切問題の結果が不安定または不適切だった場合の診断手順を提案せよ。
