...
[教科書](/textbook/frontier-long-context-memory)と対応する10問。ヒント以降は折りたたんである。

### 10-1017-01：概念

1. longを定義し、contextとの違いを説明せよ。
...
- 完全解答: longは対象と条件を定める概念であり、contextとは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: long contextとmemoryの定義と計算手順を同一視する。

</details>

### 10-1017-02：概念

2. 「long contextとmemory」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\operatorname{cost}_{attention}=O(T^2d)$$
...
4. long contextとmemoryを最小の非自明な例で計算せよ。
...
6. 「long contextとmemoryの定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにlong contextとmemoryの公式を適用した。どの項目を検査すべきか。
...
8. long contextとmemoryを実装した関数の入力・出力・検算項目を設計せよ。
...
9. long contextとmemoryがFrontierの後続Topicでどのように使われるか説明せよ。
...
- 完全解答: long contextとmemoryは、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. long contextとmemoryの結果が不安定または不適切だった場合の診断手順を提案せよ。
