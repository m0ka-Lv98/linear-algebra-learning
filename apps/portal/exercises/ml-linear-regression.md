...
[教科書](/textbook/ml-linear-regression)と対応する10問。ヒント以降は折りたたんである。

### 08-802-01：概念

1. 線形回帰を定義し、線形回帰との違いを説明せよ。
...
- 完全解答: 線形回帰は対象と条件を定める概念であり、線形回帰とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 線形回帰の定義と計算手順を同一視する。

</details>

### 08-802-02：概念

2. 「線形回帰」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\hat{y}=\mathbf{x}^{\mathsf T}\boldsymbol{\beta}+b$$
...
6. 「線形回帰の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに線形回帰の公式を適用した。どの項目を検査すべきか。
...
9. 線形回帰が機械学習の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 線形回帰は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 線形回帰の結果が不安定または不適切だった場合の診断手順を提案せよ。
