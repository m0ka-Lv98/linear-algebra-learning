...
[教科書](/textbook/opt-smoothness-strong-convexity)と対応する10問。ヒント以降は折りたたんである。

### 06-603-01：概念

1. 滑らかさを定義し、強凸性との違いを説明せよ。
...
- 完全解答: 滑らかさは対象と条件を定める概念であり、強凸性とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 滑らかさと強凸性の定義と計算手順を同一視する。

</details>

### 06-603-02：概念

2. 「滑らかさと強凸性」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\frac{\mu}{2}\|\mathbf{x}-\mathbf{y}\|_2^2\le f(\mathbf{x})-f(\mathbf{y})-\nabla f(\mathbf{y})^{\mathsf T}(\mathbf{x}-\mathbf{y})$$
...
6. 「滑らかさと強凸性の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに滑らかさと強凸性の公式を適用した。どの項目を検査すべきか。
...
8. 滑らかさと強凸性を実装した関数の入力・出力・検算項目を設計せよ。
...
9. 滑らかさと強凸性が最適化の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 滑らかさと強凸性は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 滑らかさと強凸性の結果が不安定または不適切だった場合の診断手順を提案せよ。
