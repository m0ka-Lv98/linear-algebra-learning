...
[教科書](/textbook/num-monte-carlo-methods)と対応する10問。ヒント以降は折りたたんである。

### 05-519-01：概念

1. Monteを定義し、Carlo数値計算法との違いを説明せよ。
...
- 完全解答: Monteは対象と条件を定める概念であり、Carlo数値計算法とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: Monte Carlo数値計算法の定義と計算手順を同一視する。

</details>

### 05-519-02：概念

2. 「Monte Carlo数値計算法」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\hat{I}_n=\frac{1}{n}\sum_{i=1}^{n}f(X_i)$$
...
4. Monte Carlo数値計算法を最小の非自明な例で計算せよ。
...
6. 「Monte Carlo数値計算法の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにMonte Carlo数値計算法の公式を適用した。どの項目を検査すべきか。
...
8. Monte Carlo数値計算法を実装した関数の入力・出力・検算項目を設計せよ。
...
9. Monte Carlo数値計算法が数値計算の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: Monte Carlo数値計算法は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. Monte Carlo数値計算法の結果が不安定または不適切だった場合の診断手順を提案せよ。
