...
[教科書](/textbook/mat-wls-inverse-variance)と対応する10問。ヒント以降は折りたたんである。

### 07-707-01：概念

1. WLSを定義し、逆分散重みとの違いを説明せよ。
...
- 完全解答: WLSは対象と条件を定める概念であり、逆分散重みとは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: WLSと逆分散重みの定義と計算手順を同一視する。

</details>

### 07-707-02：概念

2. 「WLSと逆分散重み」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\hat{\boldsymbol{\beta}}=(\mathbf{X}^{\mathsf T}\mathbf{W}\mathbf{X})^{-1}\mathbf{X}^{\mathsf T}\mathbf{W}\mathbf{y}$$
...
6. 「WLSと逆分散重みの定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにWLSと逆分散重みの公式を適用した。どの項目を検査すべきか。
...
8. WLSと逆分散重みを実装した関数の入力・出力・検算項目を設計せよ。
...
9. WLSと逆分散重みがデータ解析の行列手法の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: WLSと逆分散重みは、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. WLSと逆分散重みの結果が不安定または不適切だった場合の診断手順を提案せよ。
