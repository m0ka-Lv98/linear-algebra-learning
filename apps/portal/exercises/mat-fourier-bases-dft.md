...
[教科書](/textbook/mat-fourier-bases-dft)と対応する10問。ヒント以降は折りたたんである。

### 07-711-01：概念

1. Fourier基底を定義し、DFTとの違いを説明せよ。
...
- 完全解答: Fourier基底は対象と条件を定める概念であり、DFTとは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: Fourier基底とDFTの定義と計算手順を同一視する。

</details>

### 07-711-02：概念

2. 「Fourier基底とDFT」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$X_k=\sum_{n=0}^{N-1}x_n e^{-2\pi i kn/N}$$
...
5. Fourier基底に関する性質を一つ選び、定義から導出せよ。
...
6. 「Fourier基底とDFTの定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにFourier基底とDFTの公式を適用した。どの項目を検査すべきか。
...
8. Fourier基底とDFTを実装した関数の入力・出力・検算項目を設計せよ。
...
9. Fourier基底とDFTがデータ解析の行列手法の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: Fourier基底とDFTは、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. Fourier基底とDFTの結果が不安定または不適切だった場合の診断手順を提案せよ。
