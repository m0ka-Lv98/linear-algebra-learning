...
[教科書](/textbook/num-errors-conditioning-stability)と対応する10問。ヒント以降は折りたたんである。

### 05-502-01：概念

1. 誤差を定義し、条件数との違いを説明せよ。
...
- 完全解答: 誤差は対象と条件を定める概念であり、条件数とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 誤差・条件数・数値安定性の定義と計算手順を同一視する。

</details>

### 05-502-02：概念

2. 「誤差・条件数・数値安定性」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\frac{\|\Delta\mathbf{x}\|}{\|\mathbf{x}\|}\lesssim\kappa(\mathbf{A})\frac{\|\Delta\mathbf{b}\|}{\|\mathbf{b}\|}$$
...
6. 「誤差・条件数・数値安定性の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに誤差・条件数・数値安定性の公式を適用した。どの項目を検査すべきか。
...
8. 誤差・条件数・数値安定性を実装した関数の入力・出力・検算項目を設計せよ。
...
9. 誤差・条件数・数値安定性が数値計算の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 誤差・条件数・数値安定性は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 誤差・条件数・数値安定性の結果が不安定または不適切だった場合の診断手順を提案せよ。
