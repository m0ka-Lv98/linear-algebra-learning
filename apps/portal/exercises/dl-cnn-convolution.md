...
[教科書](/textbook/dl-cnn-convolution)と対応する10問。ヒント以降は折りたたんである。

### 09-906-01：概念

1. CNNを定義し、畳み込みとの違いを説明せよ。
...
- 完全解答: CNNは対象と条件を定める概念であり、畳み込みとは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: CNNと畳み込みの定義と計算手順を同一視する。

</details>

### 09-906-02：概念

2. 「CNNと畳み込み」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$y_{i,j,k}=\sum_{a,b,c}w_{a,b,c,k}x_{i+a,j+b,c}$$
...
6. 「CNNと畳み込みの定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにCNNと畳み込みの公式を適用した。どの項目を検査すべきか。
...
8. CNNと畳み込みを実装した関数の入力・出力・検算項目を設計せよ。
...
9. CNNと畳み込みが深層学習の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: CNNと畳み込みは、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. CNNと畳み込みの結果が不安定または不適切だった場合の診断手順を提案せよ。
