...
[教科書](/textbook/dl-graph-neural-networks)と対応する10問。ヒント以降は折りたたんである。

### 09-916-01：概念

1. graphを定義し、neuralとの違いを説明せよ。
...
- 完全解答: graphは対象と条件を定める概念であり、neuralとは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: graph neural networkの定義と計算手順を同一視する。

</details>

### 09-916-02：概念

2. 「graph neural network」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
4. graph neural networkを最小の非自明な例で計算せよ。
...
6. 「graph neural networkの定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにgraph neural networkの公式を適用した。どの項目を検査すべきか。
...
8. graph neural networkを実装した関数の入力・出力・検算項目を設計せよ。
...
9. graph neural networkが深層学習の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: graph neural networkは、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. graph neural networkの結果が不安定または不適切だった場合の診断手順を提案せよ。
