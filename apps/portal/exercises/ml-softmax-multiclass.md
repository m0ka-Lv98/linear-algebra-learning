...
[教科書](/textbook/ml-softmax-multiclass)と対応する10問。ヒント以降は折りたたんである。

### 08-804-01：概念

1. softmaxを定義し、多クラス分類との違いを説明せよ。
...
- 完全解答: softmaxは対象と条件を定める概念であり、多クラス分類とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: softmaxと多クラス分類の定義と計算手順を同一視する。

</details>

### 08-804-02：概念

2. 「softmaxと多クラス分類」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$p(y=k\mid\mathbf{x})=\frac{e^{z_k}}{\sum_j e^{z_j}}$$
...
4. softmaxと多クラス分類を最小の非自明な例で計算せよ。
...
6. 「softmaxと多クラス分類の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにsoftmaxと多クラス分類の公式を適用した。どの項目を検査すべきか。
...
8. softmaxと多クラス分類を実装した関数の入力・出力・検算項目を設計せよ。
...
9. softmaxと多クラス分類が機械学習の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: softmaxと多クラス分類は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. softmaxと多クラス分類の結果が不安定または不適切だった場合の診断手順を提案せよ。
