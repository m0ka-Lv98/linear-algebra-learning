...
[教科書](/textbook/num-floating-point-rounding)と対応する10問。ヒント以降は折りたたんである。

### 05-501-01：概念

1. 浮動小数点数を定義し、丸めとの違いを説明せよ。
...
- 完全解答: 浮動小数点数は対象と条件を定める概念であり、丸めとは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 浮動小数点数と丸めの定義と計算手順を同一視する。

</details>

### 05-501-02：概念

2. 「浮動小数点数と丸め」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\operatorname{fl}(x)=x(1+\delta),\quad |\delta|\le u$$
...
6. 「浮動小数点数と丸めの定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに浮動小数点数と丸めの公式を適用した。どの項目を検査すべきか。
...
8. 浮動小数点数と丸めを実装した関数の入力・出力・検算項目を設計せよ。
...
9. 浮動小数点数と丸めが数値計算の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 浮動小数点数と丸めは、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 浮動小数点数と丸めの結果が不安定または不適切だった場合の診断手順を提案せよ。
