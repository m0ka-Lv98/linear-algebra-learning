...
[教科書](/textbook/ml-anomaly-detection)と対応する10問。ヒント以降は折りたたんである。

### 08-815-01：概念

1. 異常検知を定義し、異常検知との違いを説明せよ。
...
- 完全解答: 異常検知は対象と条件を定める概念であり、異常検知とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 異常検知の定義と計算手順を同一視する。

</details>

### 08-815-02：概念

2. 「異常検知」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$s(\mathbf{x})=-\log p(\mathbf{x})$$
...
6. 「異常検知の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに異常検知の公式を適用した。どの項目を検査すべきか。
...
9. 異常検知が機械学習の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 異常検知は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 異常検知の結果が不安定または不適切だった場合の診断手順を提案せよ。
