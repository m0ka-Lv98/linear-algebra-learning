...
[教科書](/textbook/num-eigenvalue-power-qr)と対応する10問。ヒント以降は折りたたんである。

### 05-513-01：概念

1. 固有値計算を定義し、べき乗法との違いを説明せよ。
...
- 完全解答: 固有値計算は対象と条件を定める概念であり、べき乗法とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 固有値計算・べき乗法・QR法の定義と計算手順を同一視する。

</details>

### 05-513-02：概念

2. 「固有値計算・べき乗法・QR法」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\mathbf{x}_{k+1}=\frac{\mathbf{A}\mathbf{x}_k}{\|\mathbf{A}\mathbf{x}_k\|_2}$$
...
4. 固有値計算・べき乗法・QR法を最小の非自明な例で計算せよ。
...
6. 「固有値計算・べき乗法・QR法の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに固有値計算・べき乗法・QR法の公式を適用した。どの項目を検査すべきか。
...
8. 固有値計算・べき乗法・QR法を実装した関数の入力・出力・検算項目を設計せよ。
...
9. 固有値計算・べき乗法・QR法が数値計算の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 固有値計算・べき乗法・QR法は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 固有値計算・べき乗法・QR法の結果が不安定または不適切だった場合の診断手順を提案せよ。
