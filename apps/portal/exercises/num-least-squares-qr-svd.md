...
[教科書](/textbook/num-least-squares-qr-svd)と対応する10問。ヒント以降は折りたたんである。

### 05-512-01：概念

1. 最小二乗法の数値解法を定義し、最小二乗法の数値解法との違いを説明せよ。
...
- 完全解答: 最小二乗法の数値解法は対象と条件を定める概念であり、最小二乗法の数値解法とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 最小二乗法の数値解法の定義と計算手順を同一視する。

</details>

### 05-512-02：概念

2. 「最小二乗法の数値解法」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\mathbf{A}=\mathbf{Q}\mathbf{R},\quad\min\|\mathbf{R}\mathbf{x}-\mathbf{Q}^{\mathsf T}\mathbf{b}\|_2$$
...
5. 最小二乗法の数値解法に関する性質を一つ選び、定義から導出せよ。
...
6. 「最小二乗法の数値解法の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに最小二乗法の数値解法の公式を適用した。どの項目を検査すべきか。
...
8. 最小二乗法の数値解法を実装した関数の入力・出力・検算項目を設計せよ。
...
9. 最小二乗法の数値解法が数値計算の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 最小二乗法の数値解法は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 最小二乗法の数値解法の結果が不安定または不適切だった場合の診断手順を提案せよ。
