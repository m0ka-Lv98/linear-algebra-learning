...
[教科書](/textbook/num-direct-solvers-pivoting)と対応する10問。ヒント以降は折りたたんである。

### 05-509-01：概念

1. 連立方程式の直接法を定義し、pivotingとの違いを説明せよ。
...
- 完全解答: 連立方程式の直接法は対象と条件を定める概念であり、pivotingとは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 連立方程式の直接法とpivotingの定義と計算手順を同一視する。

</details>

### 05-509-02：概念

2. 「連立方程式の直接法とpivoting」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\mathbf{P}\mathbf{A}=\mathbf{L}\mathbf{U}$$
...
4. 連立方程式の直接法とpivotingを最小の非自明な例で計算せよ。
...
5. 連立方程式の直接法に関する性質を一つ選び、定義から導出せよ。
...
6. 「連立方程式の直接法とpivotingの定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに連立方程式の直接法とpivotingの公式を適用した。どの項目を検査すべきか。
...
8. 連立方程式の直接法とpivotingを実装した関数の入力・出力・検算項目を設計せよ。
...
9. 連立方程式の直接法とpivotingが数値計算の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 連立方程式の直接法とpivotingは、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 連立方程式の直接法とpivotingの結果が不安定または不適切だった場合の診断手順を提案せよ。
