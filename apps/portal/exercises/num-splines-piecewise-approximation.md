...
[教科書](/textbook/num-splines-piecewise-approximation)と対応する10問。ヒント以降は折りたたんである。

### 05-506-01：概念

1. splineを定義し、区分的近似との違いを説明せよ。
...
- 完全解答: splineは対象と条件を定める概念であり、区分的近似とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: splineと区分的近似の定義と計算手順を同一視する。

</details>

### 05-506-02：概念

2. 「splineと区分的近似」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$s_i(x)=a_i+b_i(x-x_i)+c_i(x-x_i)^2+d_i(x-x_i)^3$$
...
6. 「splineと区分的近似の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずにsplineと区分的近似の公式を適用した。どの項目を検査すべきか。
...
8. splineと区分的近似を実装した関数の入力・出力・検算項目を設計せよ。
...
9. splineと区分的近似が数値計算の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: splineと区分的近似は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. splineと区分的近似の結果が不安定または不適切だった場合の診断手順を提案せよ。
