...
[教科書](/textbook/opt-convex-sets-functions)と対応する10問。ヒント以降は折りたたんである。

### 06-602-01：概念

1. 凸集合を定義し、凸関数との違いを説明せよ。
...
- 完全解答: 凸集合は対象と条件を定める概念であり、凸関数とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 凸集合と凸関数の定義と計算手順を同一視する。

</details>

### 06-602-02：概念

2. 「凸集合と凸関数」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$f(\theta\mathbf{x}+(1-\theta)\mathbf{y})\le\theta f(\mathbf{x})+(1-\theta)f(\mathbf{y})$$
...
6. 「凸集合と凸関数の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに凸集合と凸関数の公式を適用した。どの項目を検査すべきか。
...
8. 凸集合と凸関数を実装した関数の入力・出力・検算項目を設計せよ。
...
9. 凸集合と凸関数が最適化の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 凸集合と凸関数は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 凸集合と凸関数の結果が不安定または不適切だった場合の診断手順を提案せよ。
