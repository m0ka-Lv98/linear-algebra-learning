...
[教科書](/textbook/mat-cca-multiview)と対応する10問。ヒント以降は折りたたんである。

### 07-716-01：概念

1. 正準相関分析を定義し、正準相関分析との違いを説明せよ。
...
- 完全解答: 正準相関分析は対象と条件を定める概念であり、正準相関分析とは役割または適用範囲が異なる。定義に含まれる条件を列挙し、小さな例を示す。

- よくある誤答: 正準相関分析の定義と計算手順を同一視する。

</details>

### 07-716-02：概念

2. 「正準相関分析」で入力、出力、成立条件をそれぞれ一つ挙げよ。
...
3. 次の代表式を読み、各記号の意味と出力の型を説明せよ：$$\max_{\mathbf{a},\mathbf{b}}\operatorname{corr}(\mathbf{X}\mathbf{a},\mathbf{Y}\mathbf{b})$$
...
6. 「正準相関分析の定義と計算手順を同一視する」という説明の問題点を診断せよ。
...
7. 成立条件を確認せずに正準相関分析の公式を適用した。どの項目を検査すべきか。
...
8. 正準相関分析を実装した関数の入力・出力・検算項目を設計せよ。
...
9. 正準相関分析がデータ解析の行列手法の後続Topicでどのように使われるか説明せよ。
...
- 完全解答: 正準相関分析は、後続の数値計算・データ解析・機械学習で前提となる。 現Topicの出力が後続Topicの入力・目的関数・制約・評価指標のどれになるかを具体的に述べる。
...
10. 正準相関分析の結果が不安定または不適切だった場合の診断手順を提案せよ。
