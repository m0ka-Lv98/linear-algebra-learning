# 座標降下法と共役方向：教科書

Course 06｜最適化｜Topic 10/20

## このTopicは、前の何を受けて始まるか

前Topic `opt-trust-region-methods` で得た概念を使い、ここでは 座標降下法と共役方向 へ進む。

前提として使うのは `opt-gradient-descent-convergence`、`num-iterative-linear-solvers` です。

## まず直感を作る

一階法は局所の傾きを使って下降方向を作り、step sizeが一歩の大きさを決める。



## 図の解説

<img src="/visuals/course-06/opt-coordinate-conjugate-directions.png" alt="座標降下法と共役方向の図解" style="max-height: 440px; display:block; margin:0 auto;" />

楕円等高線上で勾配降下の軌跡を追う。 楕円等高線に垂直な矢印がgradient、その反対向きが局所的な最急降下方向である。軌跡がジグザグするのは方向ごとの曲率が異なるためである。

## 記号・型・次元

- $x_j$：第j座標
- coordinate descent：他座標固定で1座標最適化
- conjugate directions：$p_i^TAp_j=0$


## 正式な定義・代表式

coordinate descentは低costな1D/小block updateを反復。quadratic SPDではA-conjugate directionを使うと既に最適化した方向を壊さず進める。

代表式は

$$
x_j\leftarrow\arg\min_z f(x_1,\ldots,z,\ldots,x_n)
$$

です。

## なぜこの式・結論になるのか

### 1. coordinate subproblem

現在xでj以外固定し、$\phi(z)=f(x_1,...,z,...,x_n)$ を最小化。

### 2. quadraticでの干渉

$f=\frac12x^TAx-b^Tx$。方向p_iとp_jがA-conjugateなら二次交差項 $p_i^TAp_j$ が0。

### 3. CGへの接続

gradientからKrylov subspace内にconjugate方向を構成するとSPD quadraticをexact arithmeticで最大nstepで解ける。

## 教科書が省略しやすい一段を補う


### coordinate descentとconjugate directionは「どの部分空間で解くか」を変える

coordinate descentは一度に1変数（またはblock）だけ動かし、その座標方向のsubproblemを解く。L1 regularizationのようにcoordinate-wise proximal updateが簡単な場合に有効。featuresが強く相関するとaxis方向がproblem geometryと合わず遅くなる。

quadratic $f(x)=\frac12x^TAx-b^Tx$ でA-SPDならA-conjugate directions $p_i^TAp_j=0$ を選ぶと、各directionの最適化が以前のdirectionを壊さない。CGはこの性質をKrylov subspace内で構成し、exact arithmeticではn step以内に解ける。



## 途中を飛ばさず全体をつなぐ

### 座標降下法と共役方向の導出を一本につなげる

coordinate descentは低costな1D/小block updateを反復。quadratic SPDではA-conjugate directionを使うと既に最適化した方向を壊さず進める。

#### 1. coordinate subproblem

まず出発点を固定する。 現在xでj以外固定し、$\phi(z)=f(x_1,...,z,...,x_n)$ を最小化。 次に必要になるのは「quadraticでの干渉」である。

#### 2. quadraticでの干渉

ここまでで得た結果を次の段階へ渡す。 $f=\frac12x^TAx-b^Tx$。方向p_iとp_jがA-conjugateなら二次交差項 $p_i^TAp_j$ が0。 次に必要になるのは「CGへの接続」である。

#### 3. CGへの接続

最後に、前二段階の結果をまとめて結論へ進む。 gradientからKrylov subspace内にconjugate方向を構成するとSPD quadraticをexact arithmeticで最大nstepで解ける。

#### 代表式へ戻す

以上をまとめた中心式は

$$
x_j\leftarrow\arg\min_z f(x_1,\ldots,z,\ldots,x_n)
$$

である。ここでは式だけを新しい事実として追加しているのではなく、上の各段階で定義した量・仮定・変形を一つの形に圧縮している。したがって式を使うときは、途中で必要だった条件が保たれている範囲までしか結論を延長できない。

### 具体例と一般式を往復する

本文の第一例は次の設定である。

Lassoでは1座標subproblemがsoft-thresholding closed formになるためcoordinate descentが効率的。

この例は特定の数値だけを覚えるためではない。上の導出で現れた量を小さい設定で実際に計算し、代表式の左辺・右辺が同じ対象を表していることを確認するためのものである。第二例では

diagonal quadraticなら座標軸自体がconjugateで、各座標を一度最適化すれば解。

と条件を変えている。二つを比較すると、数値や入力が変わっても残る構造と、仮定を変えたために変化する結論を分離できる。

### どこまで結論を信頼できるか

このTopicの境界を示す例は次である。

強くcoupledな座標でnaive coordinate descentはzig-zagし遅い。座標scale/orderも影響。

この失敗例は、単に「例外がある」という注意ではない。上の導出を逆にたどると、どの段階で必要条件が失われ、その後の式変形または解釈を続けられなくなるかを特定できる。したがって反例は定理の外側を覚えるためではなく、定理が何を仮定していたかを確認するために使う。

## 例題1：小さな数値で最後まで計算する

Lassoでは1座標subproblemがsoft-thresholding closed formになるためcoordinate descentが効率的。

## 例題2：条件を少し変えて、本質が数値依存でないことを確認する

diagonal quadraticなら座標軸自体がconjugateで、各座標を一度最適化すれば解。

## 成立条件と、条件を外したときに何が壊れるか

- step sizeが大きすぎると発散、小さすぎると遅い。
- 勾配のスケールは変数のスケールに依存する。
- 座標降下法と共役方向の定義と計算手順を区別し、数値例だけで一般性を判断しない。

強くcoupledな座標でnaive coordinate descentはzig-zagし遅い。座標scale/orderも影響。

## よくある誤解を分解する

- 座標降下法と共役方向の定義と計算手順を同一視する
- 成立条件を確認せず公式を適用する

座標降下法と共役方向では、式へ数値を代入するだけでは不十分である。強くcoupledな座標でnaive coordinate descentはzig-zagし遅い。座標scale/orderも影響。 という失敗例が示すように、式を使える条件と結論の範囲を区別する必要がある。

## 実装・数値計算では何に注意するか

random/cyclic/greedy coordinate selection、block sizeを測る。sparse featuresではupdateを局所化できる。

## ここから一段だけ発展する

ここまでunconstrained中心。次にequality constraintをLagrange multiplier/KKTで扱う。


## このTopicを理解できたか確認する問い

- 「coordinate subproblem」を式を見ずに説明できるか
- 「CGへの接続」までの論理を一段ずつ再現できるか
- 座標降下法と共役方向の条件を1つ外した反例を説明できるか

## 外部教材との照合

- [MIT OCW 6.079 Introduction to Convex Optimization](https://ocw.mit.edu/courses/6-079-introduction-to-convex-optimization-fall-2009/)
- [MIT OCW 6.7220J Nonlinear Optimization](https://ocw.mit.edu/courses/6-7220j-nonlinear-optimization-spring-2025/)
- [Boyd & Vandenberghe, Convex Optimization](https://web.stanford.edu/~boyd/cvxbook/)

[演習へ](/exercises/opt-coordinate-conjugate-directions)　|　[スライドへ](/slides/opt-coordinate-conjugate-directions/)
