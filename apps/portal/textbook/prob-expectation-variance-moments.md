...
この章では期待値・分散・モーメントを、観測値 x,y と確率変数 X,Y を区別しながら学ぶ。期待値、分散、標準偏差、モーメント、線形性、単位、典型値を、定義から小さな計算、統計・機械学習のモデルへ接続する。
...
何をランダムな対象としてモデル化し、どの量を観測データから計算するのか。期待値、分散、標準偏差、モーメント、線形性、単位、典型値はどの仮定で成立し、どこで統計推測や分類へ役立つのか。

## 記号・定義

確率は P、期待値は E、分散は Var、共分散は Covで表す。期待値、分散、標準偏差、モーメント、線形性、単位、典型値を次の式で要約する。
...
<img src="/visuals/course-03/prob-expectation-variance-moments.png" alt="期待値・分散・モーメントの図解" style="max-height: 380px; display: block; margin: 0 auto;" />
...
期待値・分散・モーメントそのものの**数学的・統計的な定義**と、それを計算するための**アルゴリズム**は別物です。同じ対象でも複数の計算法があり、計算方法を変えても定義は変わりません。逆に、似た計算手順でも前提条件が違えば別の対象を計算している場合があります。まず『何が定義なのか』を固定してから、実装を選びます。
...
同じ概念が後続では別の役割で現れます。あるCourseでは定義対象だった量が、次のCourseでは目的関数、制約、特徴量、更新則、評価指標として使われます。期待値・分散・モーメントは、後続の数値計算・データ解析・機械学習で前提となる。 この接続を意識すると、各Topicを独立した公式集ではなく、同じ数学を異なる目的で再利用する体系として理解できます。
...
期待値・分散・モーメントは、定義、support、shape、前提、数値検算を順に確認すると安全に扱える。

## 演習

[10問の演習と完全解答](/exercises/prob-expectation-variance-moments)
