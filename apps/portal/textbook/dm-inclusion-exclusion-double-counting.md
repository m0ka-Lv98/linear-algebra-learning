...
この章では包含排除原理と二重数え上げを、定義、証明、反例、アルゴリズムの仕様という順で学ぶ。2集合・3集合の包含排除、重複補正、全射、二重数え上げ、組合せ恒等式、degree sumを、数学的な主張から計算機科学の設計へ接続する。
...
2集合・3集合の包含排除、重複補正、全射、二重数え上げ、組合せ恒等式、degree sumを定義し、次の式を読み解く。
...
<img src="/visuals/course-04/dm-inclusion-exclusion-double-counting.png" alt="包含排除原理と二重数え上げの図解" style="max-height: 380px; display: block; margin: 0 auto;" />
...
包含排除原理と二重数え上げそのものの**数学的・統計的な定義**と、それを計算するための**アルゴリズム**は別物です。同じ対象でも複数の計算法があり、計算方法を変えても定義は変わりません。逆に、似た計算手順でも前提条件が違えば別の対象を計算している場合があります。まず『何が定義なのか』を固定してから、実装を選びます。
...
同じ概念が後続では別の役割で現れます。あるCourseでは定義対象だった量が、次のCourseでは目的関数、制約、特徴量、更新則、評価指標として使われます。包含排除原理と二重数え上げは、後続の数値計算・データ解析・機械学習で前提となる。 この接続を意識すると、各Topicを独立した公式集ではなく、同じ数学を異なる目的で再利用する体系として理解できます。
...
包含排除原理と二重数え上げは、定義・仮定・証明・停止性・計算量・反例を順に確認すると安全に扱える。

## 演習

[10問の演習と完全解答](/exercises/dm-inclusion-exclusion-double-counting)
