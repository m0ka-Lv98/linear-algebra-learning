...
行列をデータと線形写像として見るを、数式、shape、幾何、アルゴリズム、数値上の注意から理解する。スカラーは小文字、ベクトルは太字小文字、行列は太字大文字で表す。

## 前提知識

[la-vectors-linear-combinations](/textbook/la-vectors-linear-combinations)を前提とし、A∈R^(m×n)、x∈R^n、b∈R^mなど、初出の次元を明記する。
...
<img src="/visuals/course-02/la-matrices-data-linear-maps.png" alt="行列をデータと線形写像として見るの図解" style="max-height: 380px; display: block; margin: 0 auto;" />
...
- 行列をデータと線形写像として見るを式を見ずに一文で説明できるか。
...
行列をデータと線形写像として見るそのものの**数学的・統計的な定義**と、それを計算するための**アルゴリズム**は別物です。同じ対象でも複数の計算法があり、計算方法を変えても定義は変わりません。逆に、似た計算手順でも前提条件が違えば別の対象を計算している場合があります。まず『何が定義なのか』を固定してから、実装を選びます。
...
同じ概念が後続では別の役割で現れます。あるCourseでは定義対象だった量が、次のCourseでは目的関数、制約、特徴量、更新則、評価指標として使われます。行列をデータと線形写像として見るは、後続の数値計算・データ解析・機械学習で前提となる。 この接続を意識すると、各Topicを独立した公式集ではなく、同じ数学を異なる目的で再利用する体系として理解できます。
...
[演習へ](/exercises/la-matrices-data-linear-maps)
