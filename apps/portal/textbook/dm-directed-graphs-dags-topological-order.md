...
この章では有向グラフ・DAG・トポロジカル順序を、定義、証明、反例、アルゴリズムの仕様という順で学ぶ。directed edge、in-degree、out-degree、directed path、directed cycle、DAG（directed acyclic graph）、topological ordering、Kahn algorithmを、数学的な主張から計算機科学の設計へ接続する。
...
directed edge、in-degree、out-degree、directed path、directed cycle、DAG（directed acyclic graph）、topological ordering、Kahn algorithmを定義し、次の式を読み解く。

$$
G\text{がDAG}\Rightarrow\text{topological order}
$$
...
有向グラフ・DAG・トポロジカル順序は、定義・仮定・証明・停止性・計算量・反例を順に確認すると安全に扱える。

## 演習

[10問の演習と完全解答](/exercises/dm-directed-graphs-dags-topological-order)
