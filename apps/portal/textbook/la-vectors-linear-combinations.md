...
ベクトルと線形結合を、数式、shape、幾何、アルゴリズム、数値上の注意から理解する。スカラーは小文字、ベクトルは太字小文字、行列は太字大文字で表す。

## 前提知識

[la-symbols-types-shapes](/textbook/la-symbols-types-shapes)を前提とし、A∈R^(m×n)、x∈R^n、b∈R^mなど、初出の次元を明記する。
...
複数の方向をスカラー倍して足すことで、到達可能なベクトルの集合を作る操作として捉える。
...
<img src="/visuals/course-02/la-vectors-linear-combinations.png" alt="ベクトルと線形結合の図解" style="max-height: 380px; display: block; margin: 0 auto;" />
...
2本の2次元ベクトルを係数を変えて足し、終点が平面内をどう動くかを見る。

最小の非自明な設定で手計算し、同じ入力をNumPy等でも計算して、shape・符号・大きさ・残差・確率などを照合します。数値を変えた2例目も作り、結論が特定の数値に依存していないことを確認します。

## このTopic固有の成立条件

- 係数はスカラーであり、ベクトルの成分数は揃っている必要がある。
- 線形結合と単なる成分の並置を混同しない。
- ベクトルと線形結合の定義と計算手順を区別し、数値例だけで一般性を判断しない。
...
ベクトルと線形結合そのものの**数学的・統計的な定義**と、それを計算するための**アルゴリズム**は別物です。同じ対象でも複数の計算法があり、計算方法を変えても定義は変わりません。逆に、似た計算手順でも前提条件が違えば別の対象を計算している場合があります。まず『何が定義なのか』を固定してから、実装を選びます。
...
同じ概念が後続では別の役割で現れます。あるCourseでは定義対象だった量が、次のCourseでは目的関数、制約、特徴量、更新則、評価指標として使われます。ベクトルと線形結合は、後続の数値計算・データ解析・機械学習で前提となる。 この接続を意識すると、各Topicを独立した公式集ではなく、同じ数学を異なる目的で再利用する体系として理解できます。
...
[演習へ](/exercises/la-vectors-linear-combinations)
