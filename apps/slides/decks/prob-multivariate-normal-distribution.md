...
「多変量正規分布」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- 多変量正規分布の定義と成立条件を説明できる
- 多変量正規分布を小規模に計算・実装し検算できる

---

## まず全体像をつかむ

この章では多変量正規分布を、観測値 x,y と確率変数 X,Y を区別しながら学ぶ。確率ベクトル、平均ベクトル、共分散行列、等密度面、marginal、conditional、whiteningを、定義から小さな計算、統計・機械学習のモデルへ接続する。

---

## 記号・定義

確率は P、期待値は E、分散は Var、共分散は Covで表す。確率ベクトル、平均ベクトル、共分散行列、等密度面、marginal、conditional、whiteningを次の式で要約する。
...
多変量正規分布は、定義、support、shape、前提、数値検算を順に確認すると安全に扱える。
...
1. 多変量正規分布の定義と成立条件を説明できる
2. 多変量正規分布を小規模に計算・実装し検算できる
