...
「多変数の制約なし最適化」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- 多変数の制約なし最適化の定義と成立条件を説明できる
- 多変数の制約なし最適化を小規模に計算・実装し検算できる

---

## まず全体像をつかむ

目的関数の停留点、Hessian分類、局所最小・最大・鞍点、gradient descent、学習率η、初期値依存、scalingと収束の予告を扱う。$\nabla f(x*)=0$と更新式を示す。 数式の仮定と結論、ベクトル・行列の次元、Python/NumPyでの数値確認を区別して説明する。
...
1. 多変数の制約なし最適化の定義と成立条件を説明できる
2. 多変数の制約なし最適化を小規模に計算・実装し検算できる
