...
「主要な連続分布」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- 主要な連続分布の定義と成立条件を説明できる
- 主要な連続分布を小規模に計算・実装し検算できる

---

## まず全体像をつかむ

この章では主要な連続分布を、観測値 x,y と確率変数 X,Y を区別しながら学ぶ。一様、正規、指数、Gamma、Beta、support、PDF、locationとscaleを、定義から小さな計算、統計・機械学習のモデルへ接続する。

---

## 記号・定義

確率は P、期待値は E、分散は Var、共分散は Covで表す。一様、正規、指数、Gamma、Beta、support、PDF、locationとscaleを次の式で要約する。
...
主要な連続分布は、定義、support、shape、前提、数値検算を順に確認すると安全に扱える。
...
1. 主要な連続分布の定義と成立条件を説明できる
2. 主要な連続分布を小規模に計算・実装し検算できる
