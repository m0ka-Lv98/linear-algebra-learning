...
$\mathbb{R}^{n}\to \mathbb{R}^{m}$の写像の全微分、局所線形近似、Jacobianの行列の行・列、入力n・出力m、$m\times n$のshapeを扱う。$f(x+\Delta x)\approx f(x)+Jf(x)\Delta x$と数値確認を示す。 数式の仮定と結論、ベクトル・行列の次元、Python/NumPyでの数値確認を区別して説明する。

---

## 中心となる式・記号

- $\mathbb{R}^{n}\to \mathbb{R}^{m}$の写像の全微分
- $f(x+\Delta x)\approx f(x)+Jf(x)\Delta x$と数値確認を示す
