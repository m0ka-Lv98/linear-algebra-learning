...
「推定量・バイアス・分散・MSE」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- 推定量、バイアス、分散、MSEの定義と成立条件を説明できる
- 推定量・バイアス・分散・MSEを小規模に計算・実装し検算できる

---

## まず全体像をつかむ

この章では推定量・バイアス・分散・MSEを、観測値 x,y と確率変数 X,Y を区別しながら学ぶ。母数、推定量、推定値、バイアス、不偏性、分散、MSE、bias–variance decompositionを、定義から小さな計算、統計・機械学習のモデルへ接続する。

---

## 記号・定義

確率は P、期待値は E、分散は Var、共分散は Covで表す。母数、推定量、推定値、バイアス、不偏性、分散、MSE、bias–variance decompositionを次の式で要約する。
...
推定量・バイアス・分散・MSEは、定義、support、shape、前提、数値検算を順に確認すると安全に扱える。
...
1. 推定量、バイアス、分散、MSEの定義と成立条件を説明できる
2. 推定量・バイアス・分散・MSEを小規模に計算・実装し検算できる
