...
「離散確率・指示変数・乱択アルゴリズム」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- 離散確率、指示変数、乱択アルゴリズムの定義と成立条件を説明できる
- 離散確率・指示変数・乱択アルゴリズムを小規模に計算・実装し検算できる

---

## まず全体像をつかむ

この章では離散確率・指示変数・乱択アルゴリズムを、定義、証明、反例、アルゴリズムの仕様という順で学ぶ。有限標本空間、indicator variable I_A、期待値の線形性、randomized algorithm、expected runtime、Monte Carlo、Las Vegas、hashingを、数学的な主張から計算機科学の設計へ接続する。

---

## 記号と定義

有限標本空間、indicator variable I_A、期待値の線形性、randomized algorithm、expected runtime、Monte Carlo、Las Vegas、hashingを定義し、次の式を読み解く。
...
離散確率・指示変数・乱択アルゴリズムは、定義・仮定・証明・停止性・計算量・反例を順に確認すると安全に扱える。
...
1. 離散確率、指示変数、乱択アルゴリズムの定義と成立条件を説明できる
2. 離散確率・指示変数・乱択アルゴリズムを小規模に計算・実装し検算できる
