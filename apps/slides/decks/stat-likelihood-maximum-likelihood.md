...
「尤度と最尤推定」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- 尤度、最尤推定の定義と成立条件を説明できる
- 尤度と最尤推定を小規模に計算・実装し検算できる

---

## まず全体像をつかむ

この章では尤度と最尤推定を、観測値 x,y と確率変数 X,Y を区別しながら学ぶ。statistical model、parameter、likelihood、log-likelihood、独立標本、MLE、数値最適化を、定義から小さな計算、統計・機械学習のモデルへ接続する。

---

## 記号・定義

確率は P、期待値は E、分散は Var、共分散は Covで表す。statistical model、parameter、likelihood、log-likelihood、独立標本、MLE、数値最適化を次の式で要約する。
...
尤度と最尤推定は、定義、support、shape、前提、数値検算を順に確認すると安全に扱える。
...
1. 尤度、最尤推定の定義と成立条件を説明できる
2. 尤度と最尤推定を小規模に計算・実装し検算できる
