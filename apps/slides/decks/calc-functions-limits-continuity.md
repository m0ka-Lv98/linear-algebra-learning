...
一変数関数 f: $R\to R$ の定義域・終域・値域、グラフ、左極限・右極限、連続性を扱う。lim $x\to a$ $f(x)=L$、lim $x\to a-$、lim $x\to a+$、lim $x\to a$ $f(x)=f(a)$を区別し、穴・左右不一致・値だけ異なる例を診断する。ε-δ定義は紹介に留める。 数式の仮定と結論、ベクトル・行列の次元、Python/NumPyでの数値確認を区別して説明する。

---

## 中心となる式・記号

- 一変数関数 f: $R\to R$ の定義域・終域・値域
- lim $x\to a$ $f(x)=L$
- lim $x\to a-$
- lim $x\to a+$
- lim $x\to a$ $f(x)=f(a)$を区別し
