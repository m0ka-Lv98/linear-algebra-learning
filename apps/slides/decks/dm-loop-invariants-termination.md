...
「ループ不変量と停止性」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- ループ不変量、停止性の定義と成立条件を説明できる
- ループ不変量と停止性を小規模に計算・実装し検算できる

---

## まず全体像をつかむ

この章ではループ不変量と停止性を、定義、証明、反例、アルゴリズムの仕様という順で学ぶ。loop invariant、初期化、維持、終了時の利用、variant function、停止性、off-by-one、linear searchを、数学的な主張から計算機科学の設計へ接続する。

---

## 記号と定義

loop invariant、初期化、維持、終了時の利用、variant function、停止性、off-by-one、linear searchを定義し、次の式を読み解く。

$$
I_0\land\forall k(I_k\Rightarrow I_{k+1})
$$
...
ループ不変量と停止性は、定義・仮定・証明・停止性・計算量・反例を順に確認すると安全に扱える。
...
1. ループ不変量、停止性の定義と成立条件を説明できる
2. ループ不変量と停止性を小規模に計算・実装し検算できる
