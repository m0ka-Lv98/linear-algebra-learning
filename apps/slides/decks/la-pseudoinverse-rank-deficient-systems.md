...
「擬似逆行列とrank不足の連立方程式」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- 擬似逆行列、rank不足の連立方程式の定義と成立条件を説明できる
- 擬似逆行列とrank不足の連立方程式を小規模に計算・実装し検算できる

---

## まず全体像をつかむ

擬似逆行列とrank不足の連立方程式を、数式、shape、幾何、アルゴリズム、数値上の注意から理解する。スカラーは小文字、ベクトルは太字小文字、行列は太字大文字で表す。
...
1. 擬似逆行列、rank不足の連立方程式の定義と成立条件を説明できる
2. 擬似逆行列とrank不足の連立方程式を小規模に計算・実装し検算できる
