...
「確率変数の変換と和」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- 確率変数の変換、和の定義と成立条件を説明できる
- 確率変数の変換と和を小規模に計算・実装し検算できる

---

## まず全体像をつかむ

この章では確率変数の変換と和を、観測値 x,y と確率変数 X,Y を区別しながら学ぶ。線形変換、和の期待値、独立な和の分散、CDF法、Jacobian、convolutionを、定義から小さな計算、統計・機械学習のモデルへ接続する。

---

## 記号・定義

確率は P、期待値は E、分散は Var、共分散は Covで表す。線形変換、和の期待値、独立な和の分散、CDF法、Jacobian、convolutionを次の式で要約する。
...
確率変数の変換と和は、定義、support、shape、前提、数値検算を順に確認すると安全に扱える。
...
1. 確率変数の変換、和の定義と成立条件を説明できる
2. 確率変数の変換と和を小規模に計算・実装し検算できる
