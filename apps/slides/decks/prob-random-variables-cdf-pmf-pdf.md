...
「確率変数・CDF・PMF・PDF」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- 確率変数、CDF、PMF、PDFの定義と成立条件を説明できる
- 確率変数・CDF・PMF・PDFを小規模に計算・実装し検算できる

---

## まず全体像をつかむ

この章では確率変数・CDF・PMF・PDFを、観測値 x,y と確率変数 X,Y を区別しながら学ぶ。確率変数、離散型・連続型、PMF、PDF、CDF、support、正規化を、定義から小さな計算、統計・機械学習のモデルへ接続する。

---

## 記号・定義

確率は P、期待値は E、分散は Var、共分散は Covで表す。確率変数、離散型・連続型、PMF、PDF、CDF、support、正規化を次の式で要約する。
...
確率変数・CDF・PMF・PDFは、定義、support、shape、前提、数値検算を順に確認すると安全に扱える。
...
1. 確率変数、CDF、PMF、PDFの定義と成立条件を説明できる
2. 確率変数・CDF・PMF・PDFを小規模に計算・実装し検算できる
