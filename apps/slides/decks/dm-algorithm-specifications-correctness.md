...
「アルゴリズムの仕様と正しさ」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- アルゴリズムの仕様、正しさの定義と成立条件を説明できる
- アルゴリズムの仕様と正しさを小規模に計算・実装し検算できる

---

## まず全体像をつかむ

この章ではアルゴリズムの仕様と正しさを、定義、証明、反例、アルゴリズムの仕様という順で学ぶ。入力、出力、前提条件、事後条件、partial correctness、total correctness、停止性、反例、testとproofを、数学的な主張から計算機科学の設計へ接続する。
...
アルゴリズムの仕様と正しさは、定義・仮定・証明・停止性・計算量・反例を順に確認すると安全に扱える。
...
1. アルゴリズムの仕様、正しさの定義と成立条件を説明できる
2. アルゴリズムの仕様と正しさを小規模に計算・実装し検算できる
