...
「道・閉路・連結性」は何を表し、どの条件で使え、結果をどう検算するのか？

---

## 到達目標

- 道、閉路、連結性の定義と成立条件を説明できる
- 道・閉路・連結性を小規模に計算・実装し検算できる

---

## まず全体像をつかむ

この章では道・閉路・連結性を、定義、証明、反例、アルゴリズムの仕様という順で学ぶ。walk、trail、path、cycle、connected graph、component、distance、shortest path、BFS、Euler path、Hamilton pathを、数学的な主張から計算機科学の設計へ接続する。

---

## 記号と定義

walk、trail、path、cycle、connected graph、component、distance、shortest path、BFS、Euler path、Hamilton pathを定義し、次の式を読み解く。
...
道・閉路・連結性は、定義・仮定・証明・停止性・計算量・反例を順に確認すると安全に扱える。
...
1. 道、閉路、連結性の定義と成立条件を説明できる
2. 道・閉路・連結性を小規模に計算・実装し検算できる
